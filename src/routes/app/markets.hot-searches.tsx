...
import { HotSearchesTable } from "@/features/markets/HotSearchesTable";
...
import { gmgnHotSearchesQuery } from "@/lib/market/client";
...
export const Route = createFileRoute("/app/markets/hot-searches")({
  validateSearch: searchSchema,
  head: () => ({
    meta: [
      { title: "Robinhood Mainnet Hot Searches — RobinPulse" },
      {
        name: "description",
  ...
  component: HotSearchesPage,
});

function HotSearchesPage() {
  const { interval } = Route.useSearch();
  const navigate = Route.useNavigate();
  const query = useQuery(gmgnHotSearchesQuery(interval, 100));
    ...
    <PageContainer>
      ...
      >
        {query.isPending ? (
          ...
          <HotSearchesTable tokens={rows} interval={interval} onOpenToken={setSelected} />
        )}
      </RobinhoodMarketsPage>
      <TokenDetailDrawer
        token={selected}
