...
export function PlaceholderLegalPage({ title }: { title: string }) {
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-background px-4 py-16 text-center">
      ...
      <Link
        to="/"
        className="mt-8 inline-flex items-center rounded-full border border-border bg-card backdrop-blur-sm px-5 py-2 text-sm font-medium text-foreground transition-colors hover:bg-secondary/70"
      >
        Back to RobinPulse
      </Link>
    </main>
