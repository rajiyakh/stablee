...
| -------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| LI.FI    | `LIFI_API_KEY`, `LIFI_INTEGRATOR=robinpulse`, `LIFI_PAYOUT_CONFIRMED=true` | Register an integrator account at LI.FI's partner portal; the API key and integrator string come from there. The integrator registration is also where the payout wallet gets linked for fee payout — this is set on LI.FI's own dashboard, entirely separate from `ROBINPULSE_TREASURY_ADDRESS`. Only set `LIFI_PAYOUT_CONFIRMED=true` after manually confirming, on LI.FI's dashboard, that the registered payout wallet is exactly `ROBINPULSE_TREASURY_ADDRESS` — until then, LI.FI is excluded from results entirely, the same as an unconfirmed Relay/Across. |
