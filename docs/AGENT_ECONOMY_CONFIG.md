...
Farming is displayed to users **exclusively as a daily rate** ($ORCA/day, the `pulsePerDay` field — internal field name unchanged, display label is $ORCA) — hourly farming language ("Hourly Farming", "ORCA/hour", "Hourly Rate") must never appear on a public surface. The formula itself can still operate on a per-second basis internally; see `docs/AGENT_FARMING_CALCULATION.md`.
...
| `pulsePerDay`                                       | Recruitment card, comparison table, detail page, recruitment modal, dashboard, farming calculations (`docs/AGENT_FARMING_CALCULATION.md`) — always labeled "Daily Farming" / "$ORCA/day"                                                                                   |
